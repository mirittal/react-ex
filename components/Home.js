...
var Home = React.createClass({
	...
	componentWillMount() {
		var data = {startRankIndex:1, numMovies: this.state.numOfTopMovies};
		...
		return (
			<div className='container'>
				...
				<div className="call-to-actions">
					<Link to={'/movies/'} className="btn btn-secondary">Find More Movies</Link>
				</div>
			</div>
		);
...
var Movie =	React.createClass({
	...
	addMovie(item, i) {
  	...
  	render(){
  		//react slider settings (react-slick)
  		var settings = {
		    dots: true,
		    infinite: true,
		    speed: 700,
